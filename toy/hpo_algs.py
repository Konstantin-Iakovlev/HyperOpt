...
def loss_fn_trn(w_params, h_params, state: DataWTrainState, batch, is_training=True):
    ...
    (x, y), _ = state.apply_fn(w_params, state.bn_state, state.rng,
                                          None, is_training)
    t, _ = state.wnet_fn(h_params[0], state.bn_state, state.rng, None, is_training)

    """
    
    loss = (x-1 + jax.random.normal(state.rng) * S)**2
    loss += jnp.exp(t) * (x)**2
    """
    rasstrigin = 20 - 10 * jnp.cos(2 * jnp.pi * x) - 10 * jnp.cos(2 * jnp.pi * y)  + x**2 + y**2 + CORR * x * y
    loss = (t-y)**2 + (t - x)**2 + rasstrigin
...
def compute_metrics(state):
    (x, y),_ = state.apply_fn(state.w_params, state.bn_state, state.rng,
                                          None, False)
