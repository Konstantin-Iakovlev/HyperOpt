...
def B_jvp(params, batch, state, v, r=1e-2):
    ...
    def fun(w, h):
        return loss_fn(w, state,
                       {'image': h, 'label': batch['label']})
    dl_dlam = jax.grad(fun, argnums=1, has_aux=True)
    g_plus = dl_dlam(w_plus, batch['image'])[0]
    g_minus = dl_dlam(w_minus, batch['image'])[0]
...
def proposed_so_grad(state, batch_total, val_batch, gamma, T):
    g_so_total = jnp.zeros_like(batch_total['image'])
    for step in range(T):
        new_state = inner_step(state, batch_total)
        curr_alpha = loss_fn_grad_params(new_state.params, state, val_batch)
        g_so = B_jvp(state.params, batch_total, state, curr_alpha) * gamma ** (T - 1 - step)
        g_so_total += g_so
        state = new_state
    return state, g_so_total


def luketina_so_grad(state, batch_total, val_batch, T):
    for _ in range(T):
        state = inner_step(state, batch_total)
    curr_alpha = loss_fn_grad_params(state.params, state, val_batch)
    g_so = B_jvp(state.params, batch_total, state, curr_alpha)
    return state, g_so


def fish_so_grad(state: DataCleanTrainState, batches, val_batch, T):
...
def IFT_grad(state, batch_total, val_batch, N, T):
    for _ in range(T):
        state = inner_step(state, batch_total)
    v = loss_fn_grad_params(state.params, state, val_batch)
    so_grad = B_jvp(state.params, batch_total, state, v)
    for k in range(1, N + 1):
        v = A_jvp(state.params, batch_total, state, v)
        hvp = B_jvp(state.params, batch_total, state, v)
